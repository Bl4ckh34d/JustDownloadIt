    ...
    while tries < max_tries:
        ...
        try:
            ...
            if downloader.isSuccessful():
                ...
                while combine_tries < MAX_COMBINE_RETRIES:
                    try:
                        ...
                        root.after(0, lambda: update_progress(download_id, 100, text=f"{short_name} - Done!"))
                        print_log("Download completed successfully!")
            ...
            if tries < max_tries:
                ...
                root.after(0, lambda: update_progress(download_id, 0, text=f"{short_name} - Retry in {WAIT_BETWEEN_TRIES}s..."))
                time.sleep(WAIT_BETWEEN_TRIES)
            else:
                print_log(f"All {max_tries} attempts failed.")
                root.after(0, lambda: update_progress(download_id, 0, text=f"{short_name} - Failed"))
                current_file_var.set("")
                return False
