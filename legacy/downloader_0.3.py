...
def download_youtube_once(url: str, destination: Path):
    ...
    if combine_av_var.get():
            ...
            if d['status'] == 'downloading':
                try:
                    ...
                    if total and speed:
                        ...
                        root.after(0, download_progress_var.set, int(percentage))
                        root.after(0, pbar_label.config, {"text": progress_text, "style": "PbarOverlay.TLabel"})
                except Exception as e:
                    print_log(f"Progress error (video): {e}")
            elif d['status'] == 'finished':
                root.after(0, pbar_label.config, {"text": "Video Done - 100%", "style": "PbarOverlay.TLabel"})
            ...
            if d['status'] == 'downloading':
                try:
                    ...
                    if total and speed:
                        ...
                        root.after(0, download_progress_var_audio.set, int(percentage))
                        root.after(0, pbar_label_audio.config, {"text": progress_text, "style": "PbarOverlay.TLabel"})
                except Exception as e:
                    print_log(f"Progress error (audio): {e}")
            elif d['status'] == 'finished':
                root.after(0, pbar_label_audio.config, {"text": "Audio Done - 100%", "style": "PbarOverlay.TLabel"})
                # Hide the audio progress bar after completion
                root.after(0, download_progress_bar_audio.pack_forget)
                root.after(0, pbar_label_audio.pack_forget)

        def download_video():
            print_log('Downloading video...')
            root.after(0, download_progress_bar.pack, {'fill': 'x', 'pady': (0, 5)})  # Ensure video bar is visible
            ydl_opts_video = {
        ...
        def download_audio():
            ...
            root.after(0, download_progress_bar_audio.pack, {'fill': 'x', 'pady': (5, 0)})  # Show audio progress bar
            ydl_opts_audio = {
...
download_progress_var_audio = tk.IntVar(value=0)
...
# Audio Progress Bar and Label (Initially Hidden)
download_progress_bar_audio = ttk.Progressbar(progress_container, maximum=100,
                                              variable=download_progress_var_audio)
download_progress_bar_audio.pack(fill="x", pady=(5, 0))
download_progress_bar_audio.pack_forget()  # Hide initially

pbar_label_audio = ttk.Label(progress_container, text="", style="PbarOverlay.TLabel")
pbar_label_audio.place(relx=0.5, rely=0.5, anchor="center")
pbar_label_audio.pack_forget()  # Hide initially
