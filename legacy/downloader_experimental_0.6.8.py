...
def download_youtube_once(url: str, destination: Path, download_id: str, base_name: str = "", file_index=1, total_files=1):
    global stop_all_downloads
    
    # Check if URL is already being downloaded
    if url in active_urls:
        print_log(f"Already downloading: {url}")
    ...
    try:
        ...
        # Generate filename based on batch download settings
        if base_name and total_files > 1:
            # In batch mode with custom filename
            final_filename = f"{base_name}_{file_index}"
        elif base_name:
            # Single download with custom filename
            final_filename = base_name
        else:
            # Default to video title and ID
            final_filename = f"{video_title}_{video_id}"

        # Create unique temporary filenames for video and audio downloads
        video_path = str(destination / f"{final_filename}_video_temp_{uuid.uuid4()}.mp4")
        audio_path = str(destination / f"{final_filename}_audio_temp_{uuid.uuid4()}.m4a")
        output_path = str(destination / f"{final_filename}.mp4")
        ...
        if combine_av_var.get():
            ...
            # Create separate progress bars for video and audio
            video_bar_id = download_id
            audio_bar_id = get_next_bar_id()
            root.after(0, lambda: create_progress_bar(audio_bar_id))
                ...
                if d['status'] == 'downloading':
                    try:
                        ...
                        if total and speed:
                            ...
                            stats = f"{downloaded_mb:.1f} MB / {total_mb:.1f} MB @ {speed_str}"

                            # Update video progress bar using thread-safe method
                            root.after(0, lambda: update_progress(video_bar_id, int(percentage), speed=speed_str, text=os.path.basename(video_path)))
                    except Exception as e:
                        print_log(f"Progress error (video): {e}")
                elif d['status'] == 'finished':
                    root.after(0, lambda: update_progress(video_bar_id, 100, text=os.path.basename(video_path)))
                    video_done.set()
                ...
                if d['status'] == 'downloading':
                    try:
                        ...
                        if total and speed:
                            ...
                            root.after(0, lambda: update_progress(audio_bar_id, int(percentage), speed=speed_str, text=os.path.basename(audio_path)))
                    except Exception as e:
                        print_log(f"Progress error (audio): {e}")
                elif d['status'] == 'finished':
                    root.after(0, lambda: update_progress(audio_bar_id, 100, text=os.path.basename(audio_path)))
                    audio_done.set()

            def download_video():
                global stop_all_downloads
                if stop_all_downloads:
                    return
                print_log('Downloading video...')
                root.after(0, lambda: update_progress(video_bar_id, 0, text="Starting video download..."))
                ydl_opts_video = {
                    ...
                    'quiet': True,
                    'no_warnings': True,
                    'noprogress': True,  # Disable yt-dlp's progress to avoid temp file issues
                    'postprocessor_hooks': []  # Disable post-processing to avoid file conflicts
                }
                with yt_dlp.YoutubeDL(ydl_opts_video) as ydl:
                    active_downloads.append(ydl)
                    try:
                        ydl.download([url])
                    finally:
                        active_downloads.remove(ydl)
            ...
            def download_audio():
                ...
                root.after(0, lambda: update_progress(audio_bar_id, 0, text="Starting audio download..."))
                ydl_opts_audio = {
            ...
            # Wait for both downloads to signal completion
            video_done.wait()
            audio_done.wait()

            # Additional verification that files exist and are not empty
            if os.path.exists(video_path) and os.path.exists(audio_path):
                if os.path.getsize(video_path) > 0 and os.path.getsize(audio_path) > 0:
                    print_log("Both files downloaded successfully. Starting muxing process...")
                    root.after(0, lambda: update_progress(video_bar_id, 100, text="Muxing video and audio..."))
                    root.after(0, lambda: update_progress(audio_bar_id, 100, text="Muxing video and audio..."))
                    
                    # Process the muxing in a separate thread
                    def do_muxing():
                        try:
                            combine_files_ffmpeg(video_path, audio_path, output_path)
                            # Wait a bit before cleanup to ensure files are released
                            time.sleep(1)
                            try:
                                ...
                                print_log(f"Warning: Could not delete temporary files: {str(e)}")
                            root.after(0, lambda: update_progress(video_bar_id, 100, text="Download Complete"))
                            root.after(0, lambda: remove_progress_bar(audio_bar_id))
                        except Exception as e:
                            print_log(f"Muxing error: {str(e)}")
                            messagebox.showerror("Error", f"Failed to combine video and audio: {str(e)}")
                    
                    mux_thread = threading.Thread(target=do_muxing)
                    mux_thread.start()
                    mux_thread.join()
                else:
                    print_log("Error: One or both downloaded files are empty.")
                    messagebox.showerror("Error", "Downloaded files are empty.")
            else:
                print_log("Error: One or both files are missing for muxing.")
                messagebox.showerror("Error", "Failed to download video or audio.")
        else:
        ...
        # Always remove URL from active downloads, even if there's an error
...
def download_regular_once(
    ...
    try:
        while tries < max_tries and not stop_all_downloads:
            tries += 1
            print_log(f"Attempt {tries}/{max_tries} for {short_name}...")
            print_log("-"*10)

            downloader = SmartDL(url, str(final_path), threads=n_threads, timeout=300)
            active_downloads.append(downloader)
            
            try:
                downloader.start(blocking=False)

                while not downloader.isFinished() and not stop_all_downloads:
                ...
                if stop_all_downloads:
                    print_log(f"Download cancelled: {short_name}")
                    if download_id in progress_bars:
                        remove_progress_bar(download_id)
                    active_downloads.remove(downloader)
                    return False
...
def process_links_sequential(links: list[str]):
    def do_batch():
        ...
        # Create a list to store all download threads
        download_threads = []

        for i, link in enumerate(links, start=1):
            # Create progress bar before starting the download
            download_id = create_progress_bar()
            root.after(0, lambda: update_progress(download_id, 0))

            # Create a function for this specific download
            def download_task(link=link, i=i, download_id=download_id):
                nonlocal num_success
                try:
                    if "youtube.com" in link or "youtu.be" in link:
                        print_log(f"[{i}/{total_count}] YouTube link: {link}")
                        if download_youtube_once(link, dest, download_id, base_name, i, total_count):
                            num_success += 1
                    else:
                    ...
                    print_log(f"Error downloading {link}: {str(e)}")
                    if download_id in progress_bars:
                        remove_progress_bar(download_id)

            # Create and start a thread for this download
...
def toggle_format_dropdowns():
    ...
    """
    if combine_av_var.get():
        # Hide Combined dropdown and its label
        youtube_combined_label.grid_remove()
        youtube_combined_dropdown.grid_remove()
        # Show Video-Only and Audio-Only dropdowns
        youtube_videoonly_label.grid(row=1, column=0, sticky="e", padx=5, pady=5)
        youtube_videoonly_dropdown.grid(row=1, column=1, sticky="ew", padx=5, pady=5)
        youtube_audioonly_label.grid(row=2, column=0, sticky="e", padx=5, pady=5)
        youtube_audioonly_dropdown.grid(row=2, column=1, sticky="ew", padx=5, pady=5)
    else:
        # Show Combined dropdown and its label
        youtube_combined_label.grid(row=1, column=0, sticky="e", padx=5, pady=5)
        youtube_combined_dropdown.grid(row=1, column=1, sticky="ew", padx=5, pady=5)
        # Hide Video-Only and Audio-Only dropdowns and their labels
...
# Create a frame for the Load Streams button and Separate AV checkbox
top_controls_frame = ttk.Frame(youtube_frame)
top_controls_frame.grid(row=0, column=0, columnspan=2, padx=5, pady=5, sticky="ew")

load_streams_btn = ttk.Button(top_controls_frame, text="Load Streams", command=load_youtube_streams)
load_streams_btn.pack(side="left", padx=(0, 5))

# Checkbutton to toggle separate audio/video downloads
combine_av_chk = ttk.Checkbutton(
    top_controls_frame, 
    text="Separate AV",
    variable=combine_av_var,
    command=toggle_format_dropdowns
)
combine_av_chk.pack(side="left")
...
def shorten_display_name(filename, max_length=24):
    """Shorten a filename for display purposes only, keeping the original filename intact."""
...
def update_progress(download_id, progress, speed="", text="", segments=None):
    ...
    def _update():
        ...
        if text:
            ...
            display_text = shorten_display_name(text)
            if speed:
                label.config(text=f"{display_text} - {speed} - {progress:.1f}%")
            else:
...
active_downloads = []
active_urls = set()  # Track URLs being downloaded
...
def cancel_all_downloads():
    ...
    # Cancel all active SmartDL downloads
    for dl in active_downloads:
        if isinstance(dl, SmartDL):
            dl.stop()
    ...
    print_log("All downloads cancelled.")
